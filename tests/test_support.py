...
class TestNamingHelpers:
    ...
    async def test_custom_data_point_name(
        ...
        assert name_data.full_name == "HmIP-BSM VCU2128127 vch4"
    ...
    async def test_get_data_point_name(
        ...
        device = central.device_coordinator.get_device(address="VCU2128127")
        assert device
        channel4 = central.device_coordinator.get_channel(channel_address=f"{device.address}:5")
    ...
    async def test_get_device_name(
        ...
        central, _, _ = central_client_factory_with_homegear_client
        # The device details cache is warmed during device creation (fix: eager
        # refresh for new devices), so the recorded backend name is already cached
        # here - this is the real name from the session fixture, not the
    ...
    async def test_get_event_name(
        ...
        assert name_data.full_name == "HmIP-BSM VCU2128127 ch4 Level"
